...
getSelectedJobTitlesCount(): number {
  ...
  constructor(
    ...
    // this.selectedSizes = this.filters.include_Employee_Size;
    // console.log('Fruits after adding:', this.fruits);
  ...
  ngOnInit(): void {
    ...
    this.filterService.filters$.subscribe((filters) => {
      ...
      this.search();
      // this.search1()
      console.log('filters from filters component', this.filters);
    });
    this.search();
    ...
    if (this.filters.hasOwnProperty(key)) {
      ...
      if (Array.isArray(this.filters[key])) {
        ...
        if (!allSelected) {
          ...
          const selectedCount = this.filters[key].filter((isChecked: boolean) => isChecked).length;
          console.log(`Checkbox group '${normalizedKey}' selected count: ${selectedCount}`);
          count += selectedCount;
        }
      } else if (this.filters[key] !== '') {
        // Check if the property is meant for inclusion/exclusion and not empty
        if (normalizedKey.startsWith('include_') || normalizedKey.startsWith('exclude_')) {
          console.log(`Single filter '${normalizedKey}' applied`);
          count++;
        }
      }
    }
  }
  console.log(`Total applied filters count: ${count}`);
  return count;
}
...
removeselectedEmployeeSizes(){
  ...
  console.log(this.selectedEmployeeSizes,'selected emp sizes');
